...
const PerformanceFeatureSettings = () => {
	...
	const getPerformanceNoticeTitle = () => {
		return performance
			? __( 'Performance Enabled', 'wp-plugin-web' )
			: __( 'Performance Disabled', 'wp-plugin-web' );
	};
	...
	const getPerformanceNoticeText = () => {
		return performance
			...
			: __( 'Performance will no longer display.', 'wp-plugin-web' );
	};
	...
	useUpdateEffect( () => {
		...
		updateUI( '.wppw-app-navitem-Performance', performance );
	}, [ performance ] );
