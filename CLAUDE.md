...
- `composer run test` — run the PHPUnit integration suite via `tools/phpunit.phar` (gitignored; `composer run test:install-phpunit` downloads it). Run a single test with `php tools/phpunit.phar -c phpunit.xml --filter TestName`. First-time setup: copy `tests/phpunit/wp-tests-config-sample.php` to `tests/phpunit/wp-tests-config.php` (gitignored) and point it at a dedicated test database — never a real site DB, since WP core's test suite truncates tables on every run.
  - **Do not use `vendor/bin/phpunit`** — it's broken for this plugin. Every Newfold `wp-module-*` package self-registers via a Composer `autoload.files` entry (`bootstrap.php`) guarded by `if (!defined('ABSPATH')) return;`. The `vendor/bin/phpunit` binary requires the project's `vendor/autoload.php` internally *before* running `--bootstrap` (i.e. before `ABSPATH` exists), so those guards silently no-op and the module container never gets populated (`comingSoon`, `cachePurger`, etc. end up missing). The plain `phpunit.phar` doesn't touch `vendor/autoload.php` at all — that only happens once the plugin's own `bootstrap.php` requires it, at the correct point (`muplugins_loaded`, after `ABSPATH` is defined) — which is why `tests/phpunit/bootstrap.php` avoids eagerly requiring the autoloader itself.
